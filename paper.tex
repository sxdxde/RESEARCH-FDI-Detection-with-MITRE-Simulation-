...
\author{Sudarshan~Sudhakar CS23B2007
\thanks{Manuscript received April 2026.
S.~Sudhakar is with the Indian Institute of Information Technology, Design and Manufacturing Kancheepuram,
Chennai 600127, India, under the supervision of Dr.~Bhale Pradeepkumar Gajendra
(e-mail: cs23b2007@iiitdm.ac.in).}}

\maketitle

% ----------------------------------------------------------------
\begin{abstract}
False Data Injection~(FDI) attacks targeting Electric Vehicle~(EV)
charging metering are harder to catch than they look: an adversary
who inflates reported session energy by only $1$--$2\times$ the
normal 5-minute delivery can overbill a fleet by tens of thousands
of dollars annually while staying well below the gross-error bounds
used by existing rule-based detectors.
Labelled attack traces are rarely available in operational settings,
so supervised classifiers are largely off the table.
This work extends a NARX-based FDI detection pipeline to the
ACN-Data-Static dataset --- over 85\,000 real charging sessions from
three California sites --- and examines whether two very different
neural energy estimators can generate a clean enough estimation-error
signal for a downstream anomaly detector to act on.
The first estimator is a compact \emph{Nonlinear Auto-Regressive
with eXogenous inputs}~(NARX) network with just 171 parameters;
the second is an \emph{Attention-enhanced Bidirectional LSTM}
($\approx$2.16\,M parameters).
Both feed into an \emph{adaptive two-stage Isolation
Forest--CUSUM~(IF--CUSUM)} module.
A key issue discovered during experiments was that unconstrained
F1-maximisation on validation data silently tuned the CUSUM
threshold to stronger validation attacks, causing it to miss
roughly half of the lower-amplitude test attacks.
The fix --- a \emph{distribution-shift cap}
$h_{\rm cap}=3.5\,p_{99}(\text{EoE}_{\rm clean})$ --- recovered
up to 29 recall percentage points.
With the cap in place, the NARX pipeline reaches \textbf{97.9\%}
F1 (\textbf{99.9\%} recall, \textbf{96.0\%} precision)
and the Attention-BiLSTM variant reaches \textbf{97.0\%} F1
(\textbf{98.7\%} recall).
A naive mean predictor -- included as a sanity baseline -- scores
only \textbf{35.6\%} F1, which makes clear that the estimator
that generates the error signal matters far more than the
choice of anomaly detector.
All splits maintain strict zero session overlap between training
and evaluation sets.
\end{abstract}
...
As EV adoption has accelerated, charging networks have quietly
crossed a threshold from consumer convenience to critical grid
asset.
A typical DC fast-charger can draw 50--150\,kW, comparable to
hundreds of residential loads simultaneously; once fleets operate
at scale, their aggregate charging load becomes a dispatchable
demand-response resource worth hundreds of megawatt-hours~\cite{sun2020charging}.
Utilities in California, Texas, and the EU already incorporate
real-time EV demand telemetry into intraday load balancing,
so corrupted session data is no longer just a billing problem---it
becomes a grid-security concern.

OCPP~1.6/2.0, the dominant charging communication standard, runs
over plain HTTP or WebSocket and was designed primarily for
interoperability rather than adversarial resistance~\cite{sarieddine2023}.
An attacker with a man-in-the-middle position on this channel
can silently replace the reported 5-minute energy value $y(t)$
with an inflated figure, causing overbilling, distorting the
aggregate demand forecast, or concealing overcurrent
conditions~\cite{liu2011false}.
Session-level FDI is particularly nasty from a detection standpoint:
unlike SCADA intrusions that generate anomalous protocol traffic,
a falsified \texttt{MeterValues.req} message looks exactly like a
legitimate one to any firewall or network monitor.
...
The billing impact is only part of the picture.
When falsified session telemetry propagates to the grid scheduler,
a 25\% overestimate in aggregate EV demand forces the utility's
economic dispatch to procure more spinning reserve than necessary,
wasting money and potentially stressing local distribution
feeders with unnecessary ramp-rate commands~\cite{sun2020charging}.
At fleet scale---think a botnet that compromises hundreds of public
chargers simultaneously---the attacker could strategically time the
injections to congest specific transmission corridors or trigger
protective relay misoperations, turning a billing fraud into a
physical-grid incident.

\subsection{Why NARX and Attention-BiLSTM for Estimation?}
\label{sec:why}

\textbf{No physical model required.}
Both the NARX and BiLSTM estimators learn $\hat{y}(t)$ purely
from observed session traces, with no domain knowledge of battery
chemistry, thermal models, or charger topology encoded
explicitly~\cite{basumatary2025}.
In practice this means the same trained model can be dropped onto
a new site with different charger hardware without any
reengineering, which matters a lot given the heterogeneity of
real EV fleets.
...
\textit{Why these four features?}\ When I examined the raw EoE
traces, the clearest visual signature of a random-timestep injection
was a sudden, isolated spike: the previous step is clean so
$|\Delta\varepsilon|$ jumps sharply, and the local $z$-score
$z_{\rm loc}$ spikes well above the 20-step rolling mean.
By contrast, legitimate high-EoE events---session ramps,
boundary transitions between idle and active charging---show a
gradual build-up with moderate $|\Delta\varepsilon|$ and
$z_{\rm loc}$.
Including $\varepsilon^2$ amplifies the magnitude contrast and
gives the isolation trees additional discriminative surface.
In experiments, this 4-D IF produced near-perfect recall
($\approx100\%$)); the CUSUM stage's AND gate then recovers
precision.
...
The detector's security rests on the statistical gap between
clean and attacked EoE distributions rather than on keeping
the model weights secret.
An attacker who learns the published threshold values $h$ and $k$
could in principle target injections below those levels, but
doing so requires independently estimating $\hat{y}(t)$, which
depends on the private NARX weights.

\textbf{Adaptive attacker.}
A more capable adversary might query the deployed estimator
repeatedly to approximate $\hat{y}(t)$ and then inject only
$\tilde{y}(t)=\hat{y}(t)+\delta$ for a small $\delta>0$.
This evasion strategy is self-limiting: the smaller $\delta$ is
kept to avoid detection, the smaller the billing fraud profit.
From the numbers: $k+h\approx0.16$\,kWh sets the minimum
detectable inflation, corresponding roughly to a 23\%
per-session overcharge; an attacker who stays below this
threshold gains very little financially.
...
Both models are trained exclusively on clean, unattacked session
data using standard MSE loss:
\begin{equation}
  \mathcal{L} = \frac{1}{N}\sum_{t=1}^{N}
  \bigl(y(t) - \hat{y}(t)\bigr)^2.
  \label{eq:mse}
\end{equation}
One preprocessing choice worth noting: target values were clipped
to $[0,1]$\,kWh before MinMax scaling to remove a handful of
spuriously large readings that turned out to be meter-reset
artefacts in the raw ACN data; failing to clip these inflated
the training MSE noticeably.
Early stopping with patience~10 on validation MSE prevented
overfitting without manual epoch tuning.
...
Both NARX and BiLSTM sit extremely close to the ideal top-left
corner (AUC~$\approx0.9997$ and $0.9994$), whereas the Naive
predictor degrades sharply once recall is pushed above $\sim60\%$.
This gap reinforces the central finding: it is the quality of
the energy estimator, not the detector configuration, that
determines how separable the clean and attacked EoE distributions are.
...
Panel~(h) of Fig.~\ref{fig:compr2} isolates what each stage
actually contributes.
Stage~1 (IF alone) achieves near-perfect recall ($\approx100\%$)
but its precision sits at only $\approx41\%$ because contamination
$\rho=0.12$ causes the forest to flag roughly 12\% of clean data
as suspicious.
Stage~2 (CUSUM alone with the distribution-shift cap and
recall-priority calibration) already does much better:
recall $>99.5\%$ with precision $\approx94\%$.
What the AND gate buys is an additional 55-point precision jump
(from $\approx41\%$ to $96\%$) essentially for free in recall terms.
This strong complementarity between the two stages only exists
because the 4-D feature set gives the IF something genuinely
different from what the CUSUM threshold is testing.
...
Three observations stand out from the ablation table.
The distribution-shift cap is by far the most impactful single
change: disabling it (``CUSUM alone, no cap'') causes F1 to
collapse from 96.9\% to 75.4\%, with recall dropping 29 points
from 99.9\% to 70.7\%---exactly the calibration bias described
in Section~\ref{sec:scheme}.
Expanding the IF feature space from 1-D $\varepsilon$ to the
4-D set lifts F1 from 58.2\% to 68.7\%; the key features are
$|\Delta\varepsilon|$ and $z_{\rm loc}$, which encode the spike
sharpness that magnitude alone does not capture.
Finally, combining IF(4-D) with CUSUM and the cap pushes
precision from 94.1\% to 96.0\% without touching recall,
confirming that the AND gate cleanly suppresses Stage-1 false
alarms with no side-effect on true positives.
...
JPL's sessions are predominantly long overnight workplace charges
with low energy variance, which narrows the clean EoE distribution
and makes threshold placement easier --- hence the highest
site-level F1 of 98.5\%.
Office-01 is the hardest site: short PHEV sessions generate
frequent session-boundary transitions where EoE spikes briefly
before settling, and the IF occasionally flags these as anomalous,
holding precision at 94.2\%.
Despite this spread, recall exceeds 99.8\% at every site, which
suggests the distribution-shift cap is robustly handling the
differences in charging-population statistics across the three
deployments.
...
Both NARX and Attention-BiLSTM hold F1 above 97\% across
$\phi\in[1.0,3.0]$.
For NARX, F1 starts at 96.5\% at $\phi=1.0$ and plateaus near
98.2\% once $\phi\geq1.5$; the modest upward trend makes sense
because larger injections push the EoE further into the tail,
making individual attack steps easier to isolate.
The Naive predictor never breaks 55\%, even at $\phi=3.0$,
because its high baseline estimation error produces an EoE
distribution that heavily overlaps with the attack EoE regardless
of attack amplitude.
This tells me the dominant design decision is getting the
estimator accurate enough, not fine-tuning the anomaly detector;
in the low-amplitude regime ($\phi\leq2$), estimator MSE is the
bottleneck.
...
An end-to-end latency under 10\,ms against a 5-minute OCPP
telemetry interval means the pipeline introduces essentially
zero overhead---no pipelining or batching tricks required.
What I found notable about the edge deployment is that the
NARX's 171 multiply-accumulate operations and 10 sigmoid
evaluations fit entirely within the Cortex-M4's 32\,KB L1
cache, so the inference path never touches DRAM, which is
why sub-millisecond completion is achievable even at 168\,MHz.
...
%=================================================================
\section*{MITRE ATT\&CK Attack Simulation Overview}
\label{sec:mitre_overview}

This paper accompanies a fully instrumented real-time attack simulator
that replays ACN-Data-Static sessions while injecting synthetic False
Data Injection bursts mapped to the \textbf{MITRE ATT\&CK for ICS}
framework~\cite{mitre_ics}.
The following summarises the threat model, injection physics, emulator
design, and interactive dashboard; complete implementation details
appear in \texttt{src/attack/mitre\_fdi.py} and
\texttt{src/realtime/app.py}.

\subsection*{A.\; MITRE ATT\&CK Technique Chain}

Three ICS techniques combine to form the end-to-end attack:

\begin{enumerate}
  \item \textbf{T0830 --- Adversary-in-the-Middle (Collection).}
    The attacker establishes a man-in-the-middle position on the
    HTTP/WebSocket link between the EVSE and the cloud Charging
    Management System~(CMS).
    OCPP~1.6 messages are intercepted, modified, and re-forwarded
    in flight.
    This is the \emph{delivery vector} for all subsequent manipulation
    and requires no credentials or malware on the charging equipment.

  \item \textbf{T0836 --- Modify Parameter (Impair Process Control).}
    The primary offensive action.
    The attacker inflates the \texttt{minutesAvailable} field in the
    OCPP \texttt{StartTransaction} / \texttt{StatusNotification} message
    by $\vartheta$~minutes ($\vartheta \in [1,\,60]$ in experiments).
    The CMS trusts the falsified session record and schedules the
    charger to deliver proportionally more energy, causing a sustained
    upward shift in the EoE signal that the IF--CUSUM pipeline is
    designed to detect.

  \item \textbf{T0856 --- Spoof Reporting Message (Inhibit Response
    Function).}
    Once the CMS has accepted the falsified \texttt{minutesAvailable},
    the corrupted value propagates into the billing and analytics
    backends without any further validation.
    The invoice the driver or fleet operator receives reflects the
    inflated figure because the CMS never had access to an independent
    energy meter reading to cross-check---a structural gap in the
    current OCPP trust model.
    Scaled across many stations, this silent discrepancy adds up
    to substantial systematic fraud, independently of whatever
    operational disruption the mis-scheduling causes.
\end{enumerate}

\subsection*{B.\; Injection Physics and Emulator Model}

To avoid the \emph{zero-value blind-spot} of multiplicative injection
(where $\tilde{y}=\phi\cdot 0 = 0$ leaves no EoE trace), the emulator
uses an \textbf{additive} model:
\begin{equation}
  \tilde{y}(t) \;=\; y(t) + \underbrace{(\phi - 1)}_{\text{excess}}
  \cdot \bar{y}_{+},
  \qquad \phi = 1 + \frac{\vartheta}{\tau_{\rm ref}},
  \label{eq:additive}
\end{equation}
where $\bar{y}_{+}$ is the mean positive delivery rate of the current
session (kWh per 5-minute step) and
$\tau_{\rm ref} = 60$~min is the reference session length.
A noteworthy property of this formulation is that the excess term
$(\phi-1)\bar{y}_{+}$ is non-zero even when the charger happens to
be idle ($y(t)=0$), so every timestep within a burst contributes
a detectable EoE increment rather than silently disappearing into
a zero-delivery gap.

Attacks arrive in \textbf{bursts}:
\begin{itemize}
  \item \emph{Manual burst} --- \texttt{start\_attack($\vartheta$,
    duration)} injects exactly \texttt{duration} consecutive
    timesteps at intensity~$\vartheta$.
  \item \emph{Probabilistic auto-injection} ---
    \texttt{maybe\_inject($y$,\,$p$)} starts a new burst of duration
    $d \sim \mathcal{U}[10,25]$ with probability $p$ per step,
    modelling a realistic adversary who activates sporadically.
\end{itemize}
The \texttt{MITREFDIAttacker} class exposes an \texttt{AttackStatus}
dataclass at every timestep, carrying: active technique~ID and tactic,
current~$\vartheta$, scale~factor~$\phi$, and burst countdown.
This metadata is consumed by the dashboard to display ground-truth
attack labels and MITRE attribution in real time.

\subsection*{C.\; Real-Time Simulator and Dashboard}

The simulator is implemented as a FastAPI application with a single
persistent WebSocket connection (\texttt{/ws}).
At startup the server pre-loads both the NARX and Attention-BiLSTM
models, runs them over the estimation split, and calibrates per-model
CUSUM and IQR thresholds from the clean EoE statistics.
The WebSocket stream replays the dataset continuously at
user-selectable speed.

\medskip
\noindent\textbf{Interactive controls:}
\begin{itemize}
  \item \emph{Model selector} --- switch between NARX and
    Attention-BiLSTM live; the server restarts from $t=0$ with
    recalibrated thresholds.
  \item \emph{Speed slider} --- replay rate 1--200 timesteps/second.
  \item \emph{Attack intensity $\vartheta$} --- extra minutes injected
    (1--60~min, maps to $\phi \in [1.02,\,2.0]$ for
    $\tau_{\rm ref}=60$~min).
  \item \emph{Attack probability $p$} --- per-step auto-burst
    probability (0--50\%).
  \item \emph{Inject now / Stop} --- one-shot manual burst trigger
    and abort.
  \item \emph{Sensitivity slider (1--10)} --- scales alarm thresholds
    by $\exp\!\bigl((5-s)\times0.3\bigr)$, covering a $\approx15:1$
    dynamic range from conservative to aggressive.
  \item \emph{Scenario presets}: \textsc{Clean} ($p=0$),
    \textsc{Subtle} ($\vartheta=5$~min, $p=3\%$),
    \textsc{Moderate} ($\vartheta=20$~min, $p=8\%$),
    \textsc{Aggressive} ($\vartheta=60$~min, $p=15\%$).
\end{itemize}

\medskip
\noindent Each WebSocket frame carries: raw signals
($y_{\rm true}$, $\hat{y}$, $\tilde{y}$, EoE), CUSUM statistic
$S(t)$ and effective alarm level, IQR spike flag, combined detection
decision, active MITRE technique attribution, and a running confusion
matrix (TP, FP, FN, TN, precision, recall, F1, accuracy) updated at
every step.

\medskip
\noindent Figure~\ref{fig:ui_overview} shows a screenshot of the
dashboard during a \textsc{Moderate} scenario
($\vartheta=20$~min, $p=8\%$, NARX model active).

\begin{figure}[!t]
\centering
\begin{subfigure}{\columnwidth}
  \centering
  \includegraphics[width=\columnwidth]{Screenshot 2026-04-21 at 3.22.01 PM.jpg}
\end{subfigure}
...
\begin{subfigure}{\columnwidth}
  \centering
  \includegraphics[width=\columnwidth]{Screenshot 2026-04-21 at 3.23.22 PM.jpg}
\end{subfigure}

\caption{%
  Real-time attack simulator dashboard (\textsc{Moderate} scenario:
  $\vartheta=20$~min, $p=8\%$).
 }
...
This work set out to ask whether a two-stage IF--CUSUM detector,
fed by a lightweight neural energy estimator, could reliably
catch low-amplitude FDI on real EV charging data without ever
seeing a labelled attack example.
Testing on 333\,783 timesteps from ACN-Data-Static, the answer
turned out to be yes, provided two design decisions are in place.

\textbf{Key numerical results:}
The NARX+IF--CUSUM configuration reaches \textbf{99.57\%}
accuracy, \textbf{96.0\%} precision, \textbf{99.9\%} recall,
and \textbf{97.9\%} F1.
The Attention-BiLSTM variant scores \textbf{99.39\%} accuracy
and \textbf{97.0\%} F1.
Both beat the naive mean-predictor baseline (F1\,=\,35.6\%) by
more than 62 percentage points.
In the HIL experiment, attacks injected at timestep 50 were
flagged by the very next processing cycle --- a one-interval,
5-minute detection latency --- with no false alarms over the
preceding 50 clean samples.

\textbf{Methodological takeaways:}
The \emph{distribution-shift cap}
$h_{\rm cap}=3.5\,p_{99}(\text{EoE}_{\rm clean})$
turned out to matter more than I initially expected.
Without it, unconstrained F1-maximisation on validation data
silently tuned the CUSUM threshold to the stronger validation
attacks, causing it to miss roughly half the test injections
and dropping F1 by 21.5 points.
The \emph{4-D feature set}
$[\varepsilon,\varepsilon^2,|\Delta\varepsilon|,z_{\rm loc}]$
for the IF stage adds 10.5 F1 points over a 1-D baseline by
exploiting the temporal spike shape of random-timestep attacks;
without these extra dimensions, IF and CUSUM are effectively
testing the same thing.

\textbf{Limitations and open issues:}
The injected attacks in this study arrive at independently drawn
random timesteps, which produces the sharp $|\Delta\varepsilon|$
spikes the 4-D IF was explicitly designed to catch.
A more patient adversary who spreads the same total inflation
across many consecutive intervals would generate smaller
per-step deltas and a slower-rising $z_{\rm loc}$, partially
undermining Stage~1.
The rolling local $z$-score gives some resistance to distributed
attacks, but a dedicated window-level CUSUM statistic would
handle this more directly.

\textbf{Planned extensions:}
(1)~Incorporate cross-EVSE spatial correlation into the IF
feature set to detect simultaneously coordinated fleet attacks;
(2)~evaluate a transformer-based session encoder (e.g., Anomaly
Transformer~\cite{xu2022anomaly}) as a drop-in replacement for
the BiLSTM on cloud-tier hardware;
(3)~validate the pipeline on live OCPP traces from an operational
campus charging network;
(4)~extend the attack model to relay-type attacks that delay
rather than inflate reported energy, which produce negative EoE
spikes not caught by the current absolute-value EoE formulation.
...
[Online]. Available: \url{https://attack.mitre.org/matrices/ics/}
(accessed Apr.~2026).
